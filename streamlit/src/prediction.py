...
def run():
    ...
    try:
        with st.spinner("Waking up the inference service (may take up to a minute if idle)..."):
            _warm()
    except TimeoutError:
        st.warning("⏳ The inference service is starting up. Please wait a moment and try again.")
        return
    except ConnectionError:
        st.error("🔌 Can't reach the inference service. It may be temporarily down.")
        return
    except PermissionError:
        st.error("🔑 API authentication failed. Please contact the app maintainer.")
        return
    except RuntimeError as e:
        st.error(f"⚠️ Inference service error: {e}")
        return
    ...
    if st.button("Predict theme", type="primary"):
        ...
        try:
            ...
            st.error("🔌 Lost connection to the inference service. Please try again shortly.")
            return
        except RuntimeError as e:
            st.error(f"⚠️ {e}")
            return
